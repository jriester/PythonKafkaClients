...
def dict_to_message(obj, ctx):
    ...
    return PersonalData(message=obj['message'],
                        status=obj['status'])
...
def init_consumer_deserializer(config, topic):
    logger = logging.getLogger('CSFLEJsonConsumer')
    ...
    json_deserializer = JSONDeserializer(schema_str_value,
                                         dict_to_message,
        ...
        try:
            ...
            try:
                ...
                if msg.error() is None:
                    print(f"Message fetched from {msg.topic()}: {message.__str__()}")

            except Exception as e:
                print(f"Bad message from {msg.topic()} at offset {msg.offset()}\n{e}")
                break
...
if __name__ == '__main__':
    ...
    consumer, json_deserializer = init_consumer_deserializer(config, topic_name)
    consume(consumer, json_deserializer, topic_name)
